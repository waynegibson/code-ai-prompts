...
# VLAN gateway IPs
/ip address
add address=192.168.20.1/24 interface=vlan20-wired comment="Trusted wired VLAN gateway"
add address=192.168.25.1/24 interface=vlan25-wifi comment="Trusted Wi-Fi VLAN gateway"
...
add name=pool-vlan25 ranges=192.168.25.10-192.168.25.199
...
add name=pool-vlan60 ranges=192.168.60.10-192.168.60.49
add name=pool-vlan70 ranges=192.168.70.10-192.168.70.49

# DHCP servers
/ip dhcp-server
add name=dhcp-vlan20 interface=vlan20-wired address-pool=pool-vlan20 lease-time=8h disabled=no
add name=dhcp-vlan25 interface=vlan25-wifi address-pool=pool-vlan25 lease-time=8h disabled=no
...
add name=dhcp-vlan60 interface=vlan60-backup address-pool=pool-vlan60 lease-time=8h disabled=no
add name=dhcp-vlan70 interface=vlan70-services address-pool=pool-vlan70 lease-time=8h disabled=no

# DHCP network options
/ip dhcp-server network
add address=192.168.20.0/24 gateway=192.168.20.1 dns-server=192.168.20.1
add address=192.168.25.0/24 gateway=192.168.25.1 dns-server=192.168.25.1
